...
export function SalesTab({ bandId, onNavigateToConcerts }: { bandId: string; onNavigateToConcerts?: () => void }) {
  ...
  const createConcertNow = async () => {
    const name = prompt("Nom du concert ?");
    if (!name) return;
    const { data, error } = await supabase.from("concerts")
      .insert({ band_id: bandId, name, is_active: true })
      .select().single();
        ...
        <div className="fixed inset-0 bg-black/70 z-30 flex items-end" onClick={() => setPicker(false)}>
          <div className="w-full bg-card border-t border-border rounded-t-2xl p-4 max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={createConcertNow} className="inline-flex items-center gap-1 text-sm bg-primary text-primary-foreground px-3 py-2 rounded-md">
